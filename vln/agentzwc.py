...
class AirsimAgent:
    ...
    def makeAction(self, act_enum, **kwargs):
    ...
    def getPoseAfterAction(self, act_enum, **kwargs):
        ...
        if act_enum == 1:           # forward 10 m
            ...
            new_pos[0] = cur_pos[0] + act_step * np.cos(cur_rot[2])
            new_pos[1] = cur_pos[1] + act_step * np.sin(cur_rot[2])
        elif act_enum == 2:         # turn left by 45 degrees
            act_step = kwargs['degree'] if 'degree' in kwargs else 45
            new_rot[2] = new_rot[2] - act_step
    ...
    def teleportBackForth(self, dist=5):
        self.makeAction(1, dist=dist)


    def getPanoObs(self, view_num=12, save_dir="./assets"):
        rgb_obs = []
        dep_obs = []
        ...
        self.makeAction(3, degree=rest_angle)
    ...
    def get_current_state(self, quat=False):
        ...
        if not quat:
            ori = quaternion2eularian_angles(ori)       # [r, p, y]
        else:
            # print(ori)
